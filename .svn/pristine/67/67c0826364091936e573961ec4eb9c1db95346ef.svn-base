...
$.fn = $.prototype = {
    ...
    remove: function () {
		...
		while (elem.firstChild) {
			$.event.removeAll(elem.firstChild);
			elem.removeChild(elem.firstChild);
		};

		$.event.removeAll(elem);
...
/**
 * 搜索所有与指定CSS类匹配的元素
 * 注意：只支持.className的形式，并且只返回第一个元素
 * @param	{String}
 * @return	{this}
 */
...
$.event = {
	...
	add: function (elem, type, callback) {
		var that = this,
			...
			data = elem[expando] = elem[expando] || {},
			listener = data[type] = data[type] || {},
			guid = fn[expando] = fn[expando] || that.id ++,
			ecma = 'addEventListener' in elem,
			add = ecma ? 'addEventListener' : 'attachEvent';
		
		if (!ecma) type = 'on' + type;
		callback = function (event) {
			event = that.fix(event || window.event);
		...
		callback[expando] = guid;
		listener[guid] = callback;
	...
	removeAll: function (elem) {
		...
		for (d in data) {
			...
			for (l in type) {
				callback = type[l];
				this.remove(elem, type, callback);
			};
		};
		elem[expando] = null;
	},
...
_use = function (event) {
	...
	_dragEvent.onstart = function (x, y) {
		...
		!_isIE6 && _isLosecapture ?
			title.bind('losecapture', _dragEvent.stop) :
			_$window.bind('blur', _dragEvent.stop);
	...
	_dragEvent.onstop = function (x, y) {
		!_isIE6 && _isLosecapture ?
			title.unbind('losecapture', _dragEvent.stop) :
			_$window.unbind('blur', _dragEvent.stop);
