...
(function ($)
{
    ...
    $.ligerui.controls.Accordion.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            {
                var icon = $(box).attr("icon") || 1;
                var header = $('<div class="l-accordion-header" icon="'+icon+'"><div class="l-accordion-toggle" icon="'+icon+'"></div><div class="l-accordion-header-inner"></div></div>');
                if (i == selectedIndex)
                    $(".l-accordion-toggle", header).addClass("l-accordion-toggle-open").addClass("l-accordion-toggle-open-icon"+icon);
            ...
            $(".l-accordion-header:first").addClass("l-accordion-header-click");
            
            //add Even
            $(".l-accordion-toggle", g.accordion).each(function ()
            {
                var icon = $(this).attr("icon") || 1;
                if (!$(this).hasClass("l-accordion-toggle-open") && !$(this).hasClass("l-accordion-toggle-close"))
                {
                    $(this).addClass("l-accordion-toggle-close").addClass("l-accordion-toggle-close-icon"+icon);
                }
            ...
            {
                var icon = $(this).attr("icon") || 1;
                $(".l-accordion-header").removeClass("l-accordion-header-click");
                var togglebtn = $(".l-accordion-toggle:first", this);
                if (togglebtn.hasClass("l-accordion-toggle-close"))
                {
                    $(this).addClass("l-accordion-header-click");
                    togglebtn.removeClass("l-accordion-toggle-close").removeClass("l-accordion-toggle-close-over l-accordion-toggle-open-over").removeClass("l-accordion-toggle-close-icon"+icon);
                    togglebtn.addClass("l-accordion-toggle-open").addClass("l-accordion-toggle-open-icon"+icon);
                    
                    $(this).next(".l-accordion-content").show(p.speed).siblings(".l-accordion-content:visible").hide(p.speed);
                    
                    //$(this).siblings(".l-accordion-header").find(".l-accordion-toggle").removeClass("l-accordion-toggle-open").removeClass("l-accordion-toggle-open-icon"+icon).addClass("l-accordion-toggle-close").addClass("l-accordion-toggle-close-icon"+icon);
                    $(this).siblings(".l-accordion-header").find(".l-accordion-toggle").each(function (i, box){
                        var icon = $(box).attr("icon") || 1;
                        $(box).removeClass("l-accordion-toggle-open").removeClass("l-accordion-toggle-open-icon"+icon).addClass("l-accordion-toggle-close").addClass("l-accordion-toggle-close-icon"+icon);
                    });
                }
                else
                {
                    $(this).removeClass("l-accordion-header-click");
                    togglebtn.removeClass("l-accordion-toggle-open").removeClass("l-accordion-toggle-close-over l-accordion-toggle-open-over").removeClass("l-accordion-toggle-open-icon"+icon);
                    togglebtn.addClass("l-accordion-toggle-close").addClass("l-accordion-toggle-close-icon"+icon);
...
(function ($)
{
    ...
    $.ligerDefaults.ComboBox = {
        ...
        dest_url:null,//数据对应的url
        t_col_name:null,//该下拉框对应的数据库表字段名称
   			p_input_name:null,//该下拉框对应的上一级的下拉框文本框名称
   			c_input_name:null,//该下拉框对应的下一级的下拉框文本框名称
   			def_val:null,
        is_linkage:true//是否进行联动
    };
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
    	 setLinkage:function(isLinkage){
	    	 	var p = this.options;
	    	 	p.is_linkage=isLinkage
    	 },
        __getType: function ()
        {
            return 'ComboBox';
        ...
        _render: function ()
        {
            ...
            //下拉框
            if(p.divheight){           	
            	g.selectBox = $('<div class="l-box-select"><div class="l-box-select-inner" style="height:'+p.divheight+'"><table cellpadding="0" cellspacing="0" border="0" class="l-box-select-table"></table></div></div>');
            }else{
            ...
            if (p.selectBoxWidth)
            {
                ...
                g.defaultWidth = p.selectBoxWidth;
            }
            else
            {
                g.selectBox.css('width', g.wrapper.css('width'));
                g.defaultWidth = parseInt(g.wrapper.css('width'));
            }
            
            
            var tmpWidth=0;
            if(g.data && p.autoChangeWidth){
                for(var i=0;i<g.data.length;i++){
                    tmpWidth = g.calculateWidth(g.data[i][p.textField]);
                    if(tmpWidth>g.defaultWidth){
                        g.defaultWidth = tmpWidth;
                    }
                }
                g.selectBox.width(g.defaultWidth);
            }
            
            
        },
        calculateWidth: function(msg){
            var wordWidth = this.options.singleWorldWidth;
            return (msg.length+1)*wordWidth+1;
        },
        
        setSelectBoxWidth: function(width){
            var g = this, p = this.options;
            if (p.selectBoxWidth){
                g.selectBox.width(width);
            }else{
                g.selectBox.css('width', width);
            }
        },
        destroy: function ()
        {
        ...
        setData: function (data)
        {
            ...
            var tmpWidth=0;
            if(data && p.autoChangeWidth){
                for(var i=0;i<data.length;i++){
                    tmpWidth = g.calculateWidth(data[i][p.textField]);
                    if(tmpWidth>g.defaultWidth){
                        g.defaultWidth = tmpWidth;
                    }
                }
        ...
        _setComboBoxWidth:function(){
        	var g = this, p = this.options;
        	var treeManager = g.treeManager;
        	var defaultWidth = p.selectBoxWidth;
        	var widthwhole = this._getTreeWholeWidth();
				  var widthText = this._getTreeMaxWidth();
				  if(defaultWidth && (defaultWidth-(widthwhole-widthText)>widthText)){
					  treeManager.setTreeWidth(defaultWidth-(widthwhole-widthText));
				  }else{
					  treeManager.setTreeWidth(widthText);
				  }
				 
				  if(defaultWidth && (defaultWidth>widthwhole)){
					  g.setSelectBoxWidth(defaultWidth);
				  }else{
					  g.setSelectBoxWidth(widthwhole);
				  }
        },
        // 得到树控件整体宽度
				_getTreeWholeWidth: function(){
					var g = this;
					var step = 22;
					var maxLevel = g._getTreeMaxLevel();
					var maxWidth = g._getTreeMaxWidth();
					
					return maxWidth+maxLevel*step;
				},
				// 得到树的最大层级
		    _getTreeMaxLevel: function(){
		    	var g = this, p = this.options;
		    	var tree = g.treeManager;
					var maxLevel=0;
				...
				// 得到树控件文字宽度
				_getTreeMaxWidth: function(tree){
					var g = this, p = this.options;
        	var tree = g.treeManager;
					var maxLevel = g._getTreeMaxLevel(tree);
					var maxWidth = 50;
					var singleWorldWidth = 12;
					var setMaxWidth = function(node, level){
						var nodeWidth=0;
						if(node.text){
							nodeWidth = node.text.length*singleWorldWidth-(maxLevel-level)*22;
        ...
        setTree: function (tree)
        {
            ...
            else
            {
                tree.onSelect = function (node)
                {
                    ...
                    g._changeValue(value, text,node.ifTriggerSelect);
                };
        ...
        _dataInit: function ()
        {
            ...
            else if (g.valueField.val() != "")
            {
                ...
                {
                    if(value)
                        g.selectValueByTree(value,false);
                }
                else
                {
                    var text = g.findTextByValue(value);
        ...
        //设置值到 文本框和隐藏域
        _changeValue: function (newValue, newText,ifTriggerSelect)
        {
            var g = this, p = this.options;
            ...
            //如果传入参数ifTriggerSelect,不执行onSelect事件，避免在取消选择的时候也执行onselect方法
            if(ifTriggerSelect == false){
            }else{
        ...
        updateSelectBoxPosition: function ()
        {
            ...
            {		
                ...
                var selfheight = g.selectBox.height() + g.wrapper.outerHeight() + 4;
                if (topheight + selfheight > $(window).height() && topheight > selfheight)
                {
                    ...
                    g.selectBox.css("marginTop", -1 * (g.selectBox.height() + g.wrapper.outerHeight() + 5));
                }else{
                		g.selectBox.css({ left: g.wrapper.offset().left, top: g.wrapper.offset().top + 1 + g.wrapper.outerHeight() });
                }               
            }
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        updateSelectBoxPosition: function ()
        {
            ...
            if (p.absolute)
            {
                ...
                if (g.text.offset().top + 4 > g.dateeditor.height() && g.text.offset().top + g.dateeditor.height() + g.text.outerHeight() + 4 - $(window).scrollTop() > $(window).height())
                {
                		var a = g.dateeditor.height() + g.text.outerHeight() + 5;
                		g.dateeditor.css({ left: g.text.offset().left, top: g.text.offset().top + 1 + g.text.outerHeight() });
                    g.dateeditor.css("marginTop", -1 * (g.dateeditor.height() + g.text.outerHeight() + 5));
                    g.showOnTop = true;
                }
                else
                {
...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        ...
        position: null,     // 由于原生fixedType的有问题,现在重新设置左下角位置sw
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            var g = this, p = this.options;
            g.set(p, true);
            var dialog = $('<div class="l-dialog"><table class="l-dialog-table" cellpadding="0" cellspacing="0" border="0"><tbody><tr><td class="l-dialog-tl"></td><td class="l-dialog-tc"><div class="l-dialog-tc-inner"><div class="l-dialog-title"></div><div class="l-dialog-winbtns"><div class="l-dialog-winbtn l-dialog-close"></div></div></div></td><td class="l-dialog-tr"></td></tr><tr><td class="l-dialog-cl"></td><td class="l-dialog-cc"><div class="l-dialog-body"><div class="l-dialog-image"></div> <div class="l-dialog-content" style="padding-right:12px; height:35px; padding-bottom:10px;"></div><div class="l-dialog-buttons"><div class="l-dialog-buttons-inner"></div></td><td class="l-dialog-cr"></td></tr><tr><td class="l-dialog-bl"></td><td class="l-dialog-bc"></td><td class="l-dialog-br"></td></tr></tbody></table></div>');
            $('body').append(dialog);
            g.dialog = dialog;
            g.element = dialog[0];
            ...
            // 只支持sw
            if(p.position && p.position=='sw'){
                var left = 0;
                var top = 0;
                var width = p.width || g.dialog.width();
                if (p.slide == true) p.slide = 'fast';
                p.left = left = 0;
                p.top = top = $(window).height() - g.dialog.height() + $(window).scrollTop();
                if (left < 0) p.left = left = 0;
                if (top < 0) p.top = top = 0;
                g.dialog.css({ left: left, top: top });
            }
            ...
            g.dialog.attr("tabindex","-1").focus();
        },
        _borderX: 12,
        _borderY: 32,
            ...
            if ($.fn.ligerDrag)
                g.draggable = g.dialog.ligerDrag({ handler: '.l-dialog-title', animate: false,
                    ...
                    onStopDrag: function (current, e)
                    {
                        ...
                        // add by allen for control drag area
                        //alert(e.clientX+"##"+e.clientY);
                        if (current.target.context.offsetLeft<0){
                        	g.set({ left: 0 });
                        }
                        
                        if(current.target.context.offsetTop<0){
                            g.set({ top: 0 });
                        }
                        
                        var bodyHeight = document.body.offsetHeight;
                        /*
                        if(current.target.context.offsetTop + current.target.context.scrollHeight > bodyHeight){
                            g.set({ top: (bodyHeight-current.target.context.scrollHeight>0?bodyHeight-current.target.context.scrollHeight:0) });
                        }
                        */
                        if(current.target.context.offsetTop + 50 > bodyHeight){
                            g.set({ top: (bodyHeight-50>0?bodyHeight-50:0) });
                        }
                        var bodyWidht = document.body.offsetWidth;
                        
                        if(current.target.context.offsetLeft + current.target.context.scrollWidth > bodyWidht){
                            g.set({ left: (bodyWidht- current.target.context.scrollWidth-10>0?bodyWidht- current.target.context.scrollWidth-10:0) });
                        }
                        
                        g.dialog.show().css({ bottom: '' });
                        g._saveStatus();
                    }
                });
        },
    ...
    l.controls.Drag.ligerExtend(l.core.UIComponent, {
        ...
        _start: function (e)
        {
            ...
            for(var i=0;i<g.target.length;i++){
                if (typeof g.target[i].style.MozUserSelect!="undefined"){
                    g.target[i].style.MozUserSelect="none";
                }
            }
        },
        _drag: function (e)
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [15, 30],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
        ...
        allowHideColumn: false,                 //是否显示'切换列层'按钮-------------------------修改
        ...
        method: 'post',                         //提交方式
        async: true,
        fixedCellHeight: false,                 //是否固定单元格的高度-----------------------修改
    ...
    $.ligerDefaults.Grid.editors['date'] =
    {
        create: function (container, editParm)
        {
            ...
            var options = {
                 nowColumn: column,
                 nowRowindex: editParm.rowindex,
                 nowGrid: editParm.grid
                };
            ...
            var obj = input.ligerDateEditor(options);
            if(editParm.grid){
             if(!editParm.grid.cellArray){
                editParm.grid.cellArray=[];
             }
             if(!editParm.grid.cellArray[editParm.rowindex]){
                editParm.grid.cellArray[editParm.rowindex]=[];
             }
             editParm.grid.cellArray[editParm.rowindex][column.columnindex]=obj;
            }
            return input;
        },
    ...
    $.ligerDefaults.Grid.editors['select'] =
     $.ligerDefaults.Grid.editors['combobox'] =
     {
         create: function (container, editParm)
         {
             ...
             var obj = input.ligerComboBox(options);
             if(editParm.grid){
                if(!editParm.grid.cellArray){
                    editParm.grid.cellArray=[];
                 }
    ...
    $.ligerDefaults.Grid.editors['int'] =
     ...
     $.ligerDefaults.Grid.editors['spinner'] =
     {
         create: function (container, editParm)
         {
             ...
             var obj = input.ligerSpinner(options);
             if(editParm.grid){
    ...
    $.ligerDefaults.Grid.editors['string'] =
     $.ligerDefaults.Grid.editors['text'] = {
         create: function (container, editParm)
         {
            var column = editParm.column;
            var input;
            if(editParm.column.editor && editParm.column.editor.maxlength && editParm.column.editor.maxlength>0){
                input = $("<input type='text' class='l-text-editing' maxlength='"+editParm.column.editor.maxlength+"' />");
            }else{
                input = $("<input type='text' class='l-text-editing'/>");
            }
             container.append(input);
             
             var options = {
                 disabled: column.editor.readOnly || false,
                 nowColumn: column,
                 nowRowindex: editParm.rowindex,
                 nowGrid: editParm.grid
             };
             ...
             var obj = input.ligerTextBox(options);
             if(editParm.grid){
                if(!editParm.grid.cellArray){
                   editParm.grid.cellArray=[];
                }
     ...
     $.ligerDefaults.Grid.editors['textarea'] = {
            create: function (container, editParm)
            {
               ...
               var input;
               if(editParm.column.editor && editParm.column.editor.maxlength && editParm.column.editor.maxlength>0){
                   input = $("<textarea class='l-text-editing' maxlength='"+editParm.column.editor.maxlength+"' style='resize:none' ></textarea>");
               }else{
                   input = $("<textarea class='l-text-editing' style='resize:none'></textarea>");
               }
                container.append(input);
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm)
        {
            ...
            if (p.dataType == "local")
            {
                ...
                {
                    g.currentData = g.filteredData||{Rows:[],Total:0};
                }
                g._showData();
            }
        ...
        _allenChangeFrozonHeight: function(){
            var g = this, p = this.options;
            if($(".l-grid-body-inner", g.gridview1).length>0){
                    var gridBody1 = $(".l-grid-body-inner:first", g.gridview1);
                    var gridBody2 = $(".l-grid-body-inner:first", g.gridview2);
                    
                    var rowBody1 = $(".l-grid-body-inner:first", gridBody1);
                    var rowBody2 = $(".l-grid-body-inner:first", gridBody2);
                    
                    var trArr1 = $(".l-grid-row", gridBody1);
                    var trArr2 = $(".l-grid-row", gridBody2);
                    
                    var maxTrHeight=p.rowHeight;
                    for(var k=0; k<trArr1.length; k++){
                        maxTrHeight=p.rowHeight;
                        $($(".l-grid-row-cell-inner", trArr2[k])).each(function(i, row){
                                if($(row).height()>maxTrHeight){
                                    maxTrHeight = $(row).height();
                                }
                            });
                        $(".l-grid-row-cell-inner:first", trArr1[k]).height(maxTrHeight);
                        $(".l-grid-row-cell-inner:first", trArr1[k]).css("line-height",maxTrHeight+"px");
                        
                        var chkObj = $(".l-grid-row-cell-inner:first", trArr1[k]).children(".l-grid-row-cell-btn-checkbox");
                        if(chkObj){
                            chkObj.css("margin-top",((maxTrHeight-15)/2)+"px");
                        }
        ...
        */
        ...
        endEdit: function (rowParm)
        {
            ...
            else if (rowParm && rowParm != "undefined")
            {
                var rowdata = g.getRow(rowParm);
                if (!g.editors[rowdata['__id']]) return;
        ...
        */
        ...
        addRows: function (rowdataArr, neardata, isBefore, parentRowData,index)
        {
            var g = this, p = this.options;
            $(rowdataArr).each(function ()
            {
                g.addRow(this, neardata, isBefore, parentRowData,index);
            });
        },
        ...
        addRow: function (rowdata, neardata, isBefore, parentRowData,index)
        {
            var g = this, p = this.options;
            rowdata = rowdata || {};
            g._addData(rowdata, parentRowData, neardata, isBefore,index);
            g.reRender();
            //标识状态
        ...
        changePage: function (ctype)
        {
            ...
            if (p.dataAction != "local" && g.isDataChanged && ctype!='start')
                return false;
            p.pageCount = parseInt($(".pcontrol span", g.toolbar).html());
            switch (ctype)
            {
                case 'start': p.newPage = 1; break;
                ...
                case 'input':
                    ...
                    p.newPage = nv;
                    break;
                case 'nowpage': p.newPage = parseInt(p.page); break;
            }
            if (p.newPage == p.page && ctype!='start' && ctype != 'nowpage') return false;
            if (p.newPage == 1)
            {
        ...
        _addData: function (rowdata, parentdata, neardata, isBefore,indexParm)
        {
            var g = this, p = this.options;
        ...
        _fillGridBody: function (data, frozen)
        {
            var g = this, p = this.options;
            //加载数据 
            //var gridhtmlarr = ['<div class="l-grid-body-inner"><table class="l-grid-body-table" cellpadding=0 cellspacing=0 height="200px"><tbody>'];
        ...
        resetTotalRender: function(value){
            var g = this, p = this.options;
            $(".l-panel-bar-total", g.element).html(value);
        },
        resetOneColumnSummary: function(columnname, value){
            var g = this, p = this.options;
            var rowSummaryObj = $(".l-grid-totalsummary:first", g.element);
            if(rowSummaryObj){
                var columnObj = rowSummaryObj.children("[columnname="+columnname+"]");
                if(columnObj){
                    $(".l-grid-totalsummary-cell-inner", columnObj).html(value);
                }
            }
        },
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var rowparm in data)
            {
                ...
                if (item['__index'] % 2 == 1 && p.alternatingRow)// && !item._rowbgcolor)
                    gridhtmlarr.push(' l-grid-row-alt');
                gridhtmlarr.push('" ');  //class end
                /*
                gridhtmlarr.push(' style="');
                if(item._rowbgcolor){
                    gridhtmlarr.push(' background:'+item._rowbgcolor+';');
                }
                if(item._rowfontcolor){
                    gridhtmlarr.push(' color:'+item._rowfontcolor+';');
                }
                if(item._rowfontbolt){
                    gridhtmlarr.push(' font-weight:bold;');
                }
                gridhtmlarr.push('" ');
                */
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.isrownumber)
                    {
                        ...
                        if (p.fixedCellHeight){
                            gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px; color: #000000; font-weight:normal; " ');
                        }else{
                            gridhtmlarr.push(' style = " color: #000000; font-weight:normal; " ');
                        }
                    ...
                    //if(alertFlag){
                        gridhtmlarr.push('l-grid-row-alert ');
                    //}
					...
					var renderColor = ''; //==========单元格变色
                    if (column.renderBackground) { 
						var content = g._getCellContent(item, column);
						renderColor = column.renderBackground.call(g, content, column);
					} 
					if ($.trim(renderColor)) {
						gridhtmlarr.push(' background:' + renderColor +'; ');
					} else if(alertFlag) {
                        gridhtmlarr.push(' background:'+item._rowbgcolor+'; ');
                    }
        ...
        _applyEditor: function (obj)
        {
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                g.bind('sysEndEdit', function ()
                {
                    ...
                    {
                        //$(rowcell).addClass("l-grid-row-cell-edited"); // 编辑表格内容更新时添加红色标记样式
        ...
        // 计算指定列名的总和或平均值 
        calculateColumn: function(columnName, srcVal, type, fx){
            var g = this, p = this.options;
            ...
            if(type==='avg'){
                return g._changeDecimal(avg, fx);
            }else{
                return g._changeDecimal(sum, fx);
            }
        },
		_changeDecimal:function(x,n){
			if($.trim(x).length==0){
				return '';
			}
			var str = $.trim(x)+'';
        ...
        _onRowOver: function (rowParm, over)
        {
            ...
            if(rowParm && rowParm.attributes['style'] && rowParm.attributes['style'].nodeValue.indexOf('background')>-1){
                return;
            }
        ...
        checkAll: function(){
            ...
            var objId = g.columns[0].__domid;
            var jobjs = $(document.getElementById(objId)).parents()
            var row = null;
            for (var i = 0, l = jobjs.length; i < l; i++){
                if ($(jobjs[i]).hasClass("l-grid-hd-row")){
                    row = $(jobjs[i]);
                    break;
                }
            }
        ...
        cancleHeadCheck: function(){//zhaow-modify
            var g = this, p = this.options;           
            if(!g.columns){
                return;
            }            
            $("#"+g.id +" .l-grid-hd-row").removeClass("l-checked");            
            //for(var i=0;i<g.columns.length;i++){ //此方法当同一个页面有多个表格时，有冲突
            //    $(document.getElementById(g.columns[i].__domid)).parents(".l-grid-hd-row").removeClass("l-checked");
            //}
        },
		
		selectHeadCheck: function(){//zhaow
			var g = this, p = this.options;
			if(!g.columns){
				return;
			}			
			$("#"+g.id +" .l-grid-hd-row").addClass("l-checked");
		},
		
		resetHeadCheck: function(){//zhaow
			var g = this;
			var l1 = g.getCheckedRows().length;
			var l2 = g.getData().length;
			if (l1!=0 && l1===l2) {
				g.selectHeadCheck();
			} else {
				g.cancleHeadCheck();
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            if ($("> div[position=left]", g.layout).length > 0)
            {
                ...
                if (lefttitle)
                {
               	...
               	var image = g.left.content.attr("image");
                if (image)
                {
                    g.left.content.attr("image", "");
                    $(".l-layout-header-inner", g.left.header).html("<h2><img src='"+image+"'/><h2>");
			 ...
			 //leftmenu系统左侧菜单
            if ($("> div[position=leftmenu]", g.layout).length > 0)
            {
                g.left = $("> div[position=leftmenu]", g.layout).wrap('<div class="l-layout-leftmenu" style="left:0px;"></div>').parent();
                g.left.header = $('<div class="layout-leftmenu-header"><div class="l-layout-header-toggle"></div><div class="layout-leftmenu-header-inner"></div></div>');
                g.left.prepend(g.left.header);
                g.left.header.toggle = $(".l-layout-header-toggle", g.left.header);
                g.left.content = $("> div[position=leftmenu]", g.left);
                if (!g.left.content.hasClass("layout-leftmenu-content"))
                    g.left.content.addClass("layout-leftmenu-content");
                if (!p.allowLeftCollapse) $(".l-layout-header-toggle", g.left.header).remove();
                //set title
                var lefttitle = g.left.content.attr("title");
                if (lefttitle)
                {
                    g.left.content.attr("title", "");
                    $(".layout-leftmenu-header-inner", g.left.header).html(lefttitle);
                }
                //set width
                g.leftWidth = p.leftWidth;
                if (g.leftWidth)
			...
			//maincenter
            if ($("> div[position=maincenter]", g.layout).length > 0)
            {
                g.center = $("> div[position=maincenter]", g.layout).wrap('<div class="l-layout-maincenter" ></div>').parent();
                g.center.content = $("> div[position=maincenter]", g.center);
                g.center.content.addClass("l-layout-maincontent");
                //set title
                var centertitle = g.center.content.attr("title");
                if (centertitle)
                {
        ...
        { 
            ...
            if (typeof (p.height) == "string" && p.height.indexOf('%') > 0)
            {
                ...
                {
                    parentHeight = windowHeight;
                    parentHeight -= parseInt($('body').css('paddingTop'))||0;
                    parentHeight -= parseInt($('body').css('paddingBottom'))||0;
                }
                else
                {
            ...
            if (g.center)
            {
                ...
                if (g.left)
                {
                    if (g.isLeftCollapse)
                    {
                        ...
                        g.centerWidth -= parseInt(g.leftCollapse.css('borderLeftWidth'))||0;
                        g.centerWidth -= parseInt(g.leftCollapse.css('borderRightWidth'))||0;
                    ...
                    else
                    {
                        ...
                        g.centerWidth -= parseInt(g.left.css('borderLeftWidth'))||0;
                        g.centerWidth -= parseInt(g.left.css('borderRightWidth'))||0;
                ...
                if (g.right)
                {
                    if (g.isRightCollapse)
                    {
                        ...
                        g.centerWidth -= parseInt(g.rightCollapse.css('borderLeftWidth'))||0;
                        g.centerWidth -= parseInt(g.rightCollapse.css('borderRightWidth'))||0;
                    ...
                    else
                    {
                        ...
                        g.centerWidth -= parseInt(g.right.css('borderLeftWidth'))||0;
                        g.centerWidth -= parseInt(g.right.css('borderRightWidth'))||0;
                        g.centerWidth -= p.space;
                    }
                }
                g.centerLeft = 0;
                if (g.left)
                {
                    if (g.isLeftCollapse)
                    {
                        ...
                        g.centerLeft += parseInt(g.leftCollapse.css('borderLeftWidth'))||0;
                        g.centerLeft += parseInt(g.leftCollapse.css('borderRightWidth'))||0;
                    ...
                    else
                    {
                        ...
                        g.centerLeft += parseInt(g.left.css('borderLeftWidth'))||0;
                        g.centerLeft += parseInt(g.left.css('borderRightWidth'))||0;
                        g.centerLeft += p.space;
                    }
                }
            ...
            if (g.right)
            {
                ...
                if (g.left)
                {
                    if (g.isLeftCollapse)
                    {
                        ...
                        g.rightLeft += parseInt(g.leftCollapse.css('borderLeftWidth'))||0;
                        g.rightLeft += parseInt(g.leftCollapse.css('borderRightWidth'))||0;
                        g.rightLeft += p.space;
                    }
                    else
                    {
                        g.rightLeft += g.left.width();
                        g.rightLeft += parseInt(g.left.css('borderLeftWidth'))||0;
                        g.rightLeft += parseInt(g.left.css('borderRightWidth'))||0;
                ...
                if (g.center)
                {
                    ...
                    g.rightLeft += parseInt(g.center.css('borderLeftWidth'))||0;
                    g.rightLeft += parseInt(g.center.css('borderRightWidth'))||0;
                    g.rightLeft += p.space;
                }
...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            if(p.ischild){
            	g.tab.links = $('<div class="l-tab-links-sub"><ul style="left: 0px; "></ul></div>');
            }else{
        ...
        _applyDrag: function (tabItemDom)
        {
            ...
            var drag = $(tabItemDom).ligerDrag(
            {
                ...
                onStartDrag: function (current, e)
                {
                    if (!$(tabItemDom).hasClass("l-selected") || !$(tabItemDom).hasClass("l-selected-child")
                    		|| !$(tabItemDom).hasClass("l-selected-sub")) {
                    	return false;
                    }
                    if (e.button == 2) return false;
        ...
        getSelectedTabItemID: function ()
        {
            ...
            return $("li.l-selected", g.tab.links.ul).attr("tabid") 
            		|| $("li.l-selected-child", g.tab.links.ul).attr("tabid")
            		|| $("li.l-selected-sub", g.tab.links.ul).attr("tabid");
        },
        ...
        selectTabItem: function (tabid)
        {
            ...
            if(tabid>=20000){//子页面弹出的tab页
            	$("li[tabid=" + tabid + "]", g.tab.links.ul).addClass("l-selected-sub").removeClass("l-unselected-sub").siblings().removeClass("l-selected-sub").addClass("l-unselected-sub");
            }else if(tabid && (""+tabid).indexOf('linktab') != -1){//子页面弹出的tab页
            	$("li[tabid=" + tabid + "]", g.tab.links.ul).addClass("l-selected-sub").removeClass("l-unselected-sub").siblings().removeClass("l-selected-sub").addClass("l-unselected-sub");
            }else if(tabid>10000){//主页面弹出的tab页
            	$("li[tabid=" + tabid + "]", g.tab.links.ul).addClass("l-selected-child").removeClass("l-unselected-child").siblings().removeClass("l-selected");
            }else{
            	$("li[tabid=" + tabid + "]", g.tab.links.ul).addClass("l-selected").siblings().removeClass("l-selected");
            	$("li[tabid=" + tabid + "]", g.tab.links.ul).addClass("l-selected").siblings().removeClass("l-selected-child").addClass("l-unselected-child");
            }
        ...
        //在子页面弹出tab页
        addTabItem_sub: function (options)
        {
            var g = this, p = this.options;
            if (g.trigger('beforeAddTabItem', [tabid]) == false)
                return false;
            ...
            var tabitem = $("<li><a></a><div class='l-tab-links-item-left-sub'></div><div class='l-tab-links-item-right-sub'></div><div class='l-tab-links-item-close'></div></li>");
            var contentitem = $("<div class='l-tab-content-item'><div class='l-tab-loading' style='display:block;'></div><iframe frameborder='0'></iframe></div>");
            var iframeloading = $("div:first", contentitem);
            var iframe = $("iframe:first", contentitem);
            if (g.makeFullHeight)
        ...
        addTabItem_child: function (options)
        {
            var g = this, p = this.options;
            if (g.trigger('beforeAddTabItem', [tabid]) == false)
                return false;
            ...
            var tabitem = $("<li><a></a><div class='l-tab-links-item-left-child'></div><div class='l-tab-links-item-right-child'></div><div class='l-tab-links-item-close'></div></li>");
            var contentitem = $("<div class='l-tab-content-item'><div class='l-tab-loading' style='display:block;'></div><iframe frameborder='0'></iframe></div>");
            var iframeloading = $("div:first", contentitem);
            var iframe = $("iframe:first", contentitem);
            if (g.makeFullHeight)
        ...
        removeTabItem: function (tabid)
        {
            ...
            var currentIsSelected = $("li[tabid=" + tabid + "]", g.tab.links.ul).hasClass("l-selected") 
            						|| $("li[tabid=" + tabid + "]", g.tab.links.ul).hasClass("l-selected-child")
            						|| $("li[tabid=" + tabid + "]", g.tab.links.ul).hasClass("l-selected-sub");
            ...
            $("> li", g.tab.links.ul).each(function ()
            {
                if ($(this).attr("tabid")
                        ...
                        && (!noclose || $(".l-tab-links-item-close", this).length > 0
                        		||  $(".l-tab-links-item-close-child", this).length > 0
                        		||  $(".l-tab-links-item-close-sub", this).length > 0))
                {
    ...
    $.ligerui.controls.Tip.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
        	...
        	if(p.isabsolute){//是否相对定位
                ...
                p.width && tip.width(p.width - 8);
                p.height && tip.height(p.height);
                eee = p.appendIdTo;
                if (p.appendIdTo)
                {
...
(function ($)
{
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item)
        {
            ...
            item.click && ditem.click(function () { item.click(item,g.id); $(this).removeClass("l-panel-btn-over");});
            ditem.hover(function ()
            {
                $(this).addClass("l-panel-btn-over");
            }, function ()
...
*/
/**
* Allen(wangyc) add a new class l-checkbox-disable, in function it will find this class and do some control functions
* if u set the item's property isdisable is true, this will be done.
* 
*/
(function ($)
{
    ...
    $.ligerDefaults.Tree = {
        specialFlag: false,  // 为true的话，选择子节点自动选中父节点，所有子节点取消不取消父节点，选中父节点自动选中所有子节点
        onlySelectFather: false,
        allCheckFlag: false, // true表示全部使用选中框
        url: null,
        data: null,
        checkbox: true,
        autoCheckboxEven: true, //自动向上、向下选择事件
        parentIcon: 'folder',
        childIcon: 'leaf',
        textFieldName: 'text',
        attribute: ['id', 'url'],
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        getChecked: function ()
        {
            ...
            $(".l-checkbox-disable", g.tree).parent().parent("li").each(function ()
            {
                var treedataindex = parseInt($(this).attr("treedataindex"));
                nodes.push({ target: this, data: g._getDataNodeByTreeDataIndex(g.data, treedataindex) });
            });
        ...
        cancelSelect: function (nodeParm)
        {
            ...
            if (p.checkbox){
                if(!$(".l-checkbox", treeitembody).hasClass("l-checkbox-disable")){
        ...
        selectNode: function (selectNodeParm,ifTriggerSelect)
        {
            var g = this, p = this.options;
            var clause = null;
            ...
            else if (typeof (selectNodeParm) == "object")
            {
                ...
                g.trigger('select', [{ data: treenodedata, target: treeitem[0],'ifTriggerSelect':ifTriggerSelect}]);
                return;
            }
            else
            {
            ...
            $("li", g.tree).each(function ()
            {
                ...
                {
                    g.selectNode(this,ifTriggerSelect);
                }
                else
                {
                    g.cancelSelect(this,ifTriggerSelect);
                }
            });
        },
                ...
                if (g.hasChildren(o))
                {
                    ...
                    if (p.checkbox)
                    {
                        if(o.isnoncheck){
                            ...
                            treehtmlarr.push('<div class="l-box l-checkbox l-checkbox-disable"></div>');
                        }else{
                            if (o.ischecked)
        ...
        _setTreeEven: function ()
        {
            ...
            { 
                ...
                if ($(obj).hasClass("l-checkbox"))
                {
                    ...
                    {
                        if (!$(obj).hasClass("l-checkbox-disable")){
                            //状态：未选中
                            if ($(obj).hasClass("l-checkbox-unchecked"))
                            {
                                ...
                                $(".l-checkbox-disable", treeitem).removeClass("l-checkbox-checked");    
                            ...
                            else if ($(obj).hasClass("l-checkbox-checked"))
                            {
                                ...
                                $(".l-checkbox-disable", treeitem).removeClass("l-checkbox-unchecked");   
        ...
        {            
            ...
            //当前同级别或低级别的节点是否都没有选中
            var isCheckedNull = ($(".l-checkbox-checked", treeitem.parent()).length == 0 && $(".l-checkbox-disable", treeitem.parent()).length == 0);
            
            var parent;
            
            if (isCheckedComplete)
            {
                parent = treeitem.parent().prev().find(".l-checkbox");
                if (!parent.hasClass("l-checkbox-disable")){
                    parent.removeClass("l-checkbox-unchecked l-checkbox-incomplete").addClass("l-checkbox-checked");
                }
            ...
            {
                parent = treeitem.parent().prev().find("> .l-checkbox");
                if (!parent.hasClass("l-checkbox-disable") && !p.specialFlag){
                    parent.removeClass("l-checkbox-checked l-checkbox-incomplete").addClass("l-checkbox-unchecked");
                }
            }
            else
            {
                parent = treeitem.parent().prev().find("> .l-checkbox");
                if (!parent.hasClass("l-checkbox-disable")){
                    if(p.specialFlag || p.allCheckFlag){
                        parent.removeClass("l-checkbox-unchecked l-checkbox-incomplete").addClass("l-checkbox-checked");
                    }else{
                        parent.removeClass("l-checkbox-unchecked l-checkbox-checked").addClass("l-checkbox-incomplete");
                    }
            ...
            if (treeitem.parent().parent("li").length > 0 && !p.onlySelectFather)
        ...
        checkParentByChildItem: function(treeitem, level, type){
            var g = this, p = this.options;
            var parent, parentItem, pindex, pdata;
            if (level == undefined){
                if(type == undefined){
                    level = 100;
                }else{
            ...
            if(treeitem){
                parent = treeitem.parent().prev().find("> .l-checkbox");
                parentItem = treeitem.parent().parent("li");
                if (!parent.hasClass("l-checkbox-disable")){
                    parent.removeClass("l-checkbox-unchecked l-checkbox-incomplete l-checkbox-checked").addClass("l-checkbox-checked");
                }
                
                if(type){
                    parentItem = treeitem.parent().parent("li");
                    if(parentItem && parentItem.length>0){
                        pindex = parentItem.attr("treedataindex");
                        pdata = g._getDataNodeByTreeDataIndex(g.data, pindex);
                        if(pdata['type'].toString()===type.toString()){
                            g.checkParentByChildItem(parentItem,level-1,type);
                        }else{
                            g.checkParentByChildItem(parentItem,level,type);
                        }
                    }
                    
                }else{
                    if(level>1 && parentItem && parentItem.length>0){
                        g.checkParentByChildItem(parentItem,level-1,null);
                    }
                }
            }else{
                return;
