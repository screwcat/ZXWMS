			...
			<c:forEach items="${workflowInfoHelps}" var="taskinfo">
				<tr>
					...
					<td>
						...
						<c:if test="${not empty taskinfo.assignee }">--%> <a
						href='<%=request.getContextPath() %>/workflow/get-form/task/${taskinfo.taskId}/${taskinfo.processInstanceId}/${taskinfo.processDefinitionKey}'>办理</a>
						<a
						href='<%=request.getContextPath() %>/workflow/task/delegateTask/${taskinfo.taskId}'>委派</a>
						<%--</c:if>--%>
					</td>
				</tr>
			</c:forEach>
