...
/**
 * 赎回流程 联动特批申请操作 实现副经理审批fzjlsp系统自动审批操作
 * 
 * @author Hancd
 */
public class FinancialFZJLApprovalProcessor implements TaskListener
{
    ...
    public void notify(DelegateTask delegateTask)
    {
        ...
        // 接收特批申请监听传递的tppass参数，判断是否执行系统自动执行
        if (delegateTask.getVariable("tppass") != null && delegateTask.getVariable("tppass").toString().equals("TP"))
        {
            if (StringUtils.isEmpty(delegateTask.getAssignee()))
            {
                ...
                taskService.setVariable(delegateTask.getId(), "fzjlsppass", "system");
                // 实现代办任务
